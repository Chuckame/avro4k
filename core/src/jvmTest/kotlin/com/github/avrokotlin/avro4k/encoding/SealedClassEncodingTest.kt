...
internal class SealedClassEncodingTest : StringSpec({
    "encode/decode sealed classes" {
        AvroAssertions.assertThat(ReferencingSealedClass(Operation.Binary.Add(1, 2)))
            .isEncodedAs(record(recordWithSchema(Avro.apacheSchema<Operation.Binary.Add>(), 1, 2)))
        AvroAssertions.assertThat<Operation>(Operation.Binary.Add(1, 2))
            .isEncodedAs(recordWithSchema(Avro.apacheSchema<Operation.Binary.Add>(), 1, 2))
    }
    "encode/decode sealed class union with value class primitive, enum and record subtypes" {
        val shapeSchema = Avro.apacheSchema<Shape>()
        ...
        AvroAssertions.assertThat<Shape>(Shape.Circle(5)).isEncodedAs(recordWithSchema(Avro.apacheSchema<Shape.Circle>(), 5))
    }
    ...
    "decode polymorphic type when schema alias matches kotlin serial name" {
        val currentSchema = Avro.apacheSchema<Polygon>()
        ...
        val encodedBytes = Avro.encodeWith(currentSchema, Avro.serializersModule.serializer<Polygon>(), Polygon.Trapezoid(4))
        val decoded = Avro.decodeWith(legacySchema, Avro.serializersModule.serializer<Polygon>(), encodedBytes)
    ...
    "decode polymorphic type when schema name matches kotlin @AvroAlias" {
        ...
        val encodedBytes = Avro.encodeWith(currentSchema, Avro.serializersModule.serializer<Polygon>(), Polygon.Hexagon(6))
        val decoded = Avro.decodeWith(legacySchema, Avro.serializersModule.serializer<Polygon>(), encodedBytes)
