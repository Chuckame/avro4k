...
class AvroGeneratedTest : StringSpec() {
    init {
        "a class with an unrelated schema in @AvroGenerated should return that schema even if invalid" {
            Avro.apacheSchema<TestValueWithUnrelatedSchema>() shouldBe Schema.Parser().parse(SCHEMA)
            Avro.apacheSchema<TestValueWithUnrelatedSchema?>() shouldBe Schema.Parser().parse(SCHEMA).nullable
        }
    }
}
