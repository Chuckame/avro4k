...
| M1 · C2 workflow memoization | merged, unmeasured | `perf/baseline` | `e79c758` | 16-entry copy-on-write identity cache in front of `fieldCache`; `WeakKeyCache` purge moved to the put path. Tests mutation-verified. See [`notes/c2.md`](notes/c2.md) |
| M1 · C3 collection serializer cache | merged, unmeasured | `perf/baseline` | `b6f0fd3` | one-entry inline cache replaces the per-collection wrapper allocation. **Read path only** — the encode path never wrapped collections. See [`notes/c3.md`](notes/c3.md) |
| M1 · C9 fingerprint memo + enum boxing | merged, unmeasured | `perf/baseline` | `7dc1872` | fingerprint memoized per-instance (weak); enum default unboxed; generic map pairs removed. 4th bullet skipped with reasons. **No benchmark covers any of it.** See [`notes/c9.md`](notes/c9.md) |
