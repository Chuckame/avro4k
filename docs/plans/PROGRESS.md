...
**M0 and M1 are complete.** Next: **M2 — KMP foundation** (B0, B1, B6).
...
| Backlog · `MapGenericDecoder` key/value tagging bug | todo | — | — | **pre-existing**, found during C9, preserved verbatim. Own commit + tests |
| Backlog · `benchmarkAlloc` fails silently | todo | — | — | a run whose forks all die reports `BUILD SUCCESSFUL` with an empty table. Assert the report has result rows |
