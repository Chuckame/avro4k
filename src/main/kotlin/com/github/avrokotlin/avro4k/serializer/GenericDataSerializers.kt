...
internal val GenericDataSerializersModule: SerializersModule =
    SerializersModule {
...
public object GenericDataSerializer : AvroSerializer<Any>("GenericData") {
    private val mapSerializer: KSerializer<Any> = MapSerializer(GenericDataSerializer, GenericDataSerializer.nullable) as KSerializer<Any>
    private val listSerializer: KSerializer<Any> = ListSerializer(GenericDataSerializer.nullable) as KSerializer<Any>
    private val arraySerializer: KSerializer<Any> = ArraySerializer(Any::class, GenericDataSerializer.nullable) as KSerializer<Any>

    override fun serializeAvro(
        encoder: AvroEncoder,
        value: Any,
    ) {
        encoder.encodeSerializableValue(findSerializer(encoder, value), value)
    }
    ...
    private fun <T : Any> findSerializer(
        ...
        return when (value) {
            ...
            else -> encoder.serializersModule.serializerOrNull(value::class.java) ?: throw SerializationException("Could not find serializer from generic value of type ${value::class.qualifiedName}")
        }
    }
    ...
    override fun deserializeAvro(decoder: AvroDecoder): Any {
        ...
        return when (decoder.currentWriterSchema.type) {
            ...
            Schema.Type.ENUM -> decoder.decodeSerializableValue(GenericEnumSerializer)
            Schema.Type.RECORD -> decoder.decodeSerializableValue(GenericRecordSerializer)
            Schema.Type.ARRAY -> decoder.decodeSerializableValue(listSerializer)
            Schema.Type.MAP -> decoder.decodeSerializableValue(mapSerializer)
...
public object GenericArraySerializer : AvroSerializer<GenericArray<*>>(GenericArray::class.java.name) {
    ...
    override fun deserializeAvro(decoder: AvroDecoder): GenericArray<*> {
        return GenericData.Array(decoder.currentWriterSchema, decoder.decodeSerializableValue(listSerializer))
    }
